...
Shows usage of CDI (Injection of managed beans) and producers together with simple servlets. This is a good sample _if you want to know about producers_. Otherwise, check a simpler CDI sample (see <<other-samples>>).
...
** Check out https://github.com/wildfly/quickstart/tree/10.x/cdi-injection[cdi-injection] for a very simple use of CDI
* GlassFish comes with samples (in the `samples` subdir of GlassFish), also available through https://svn.java.net/svn/glassfish-samples~svn/trunk/ws/javaee7/[svn]
